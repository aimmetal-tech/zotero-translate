...
- 第一条真实消息会从 Zotero 当前附件的跨平台绝对路径读取源 PDF，确认文件存在后原子复制为专用工作区中的 `source.pdf`，再以 `application/pdf` 的 ACP `resource_link` 引用；支持 macOS/Unix 路径以及 Windows 盘符和 UNC 路径。后续 turn 不重复附加 PDF，只发送文本以及用户本轮明确添加的截图图片块。
- `pdftotext` 探测按当前平台读取 PATH：Windows 查找 `pdftotext.exe`，macOS/Unix 保留 Homebrew 与系统常见位置；候选无效、不可读或不存在时安全回退 Zotero PDFWorker，不会因其他平台的路径格式阻断发送。
- Zotero 重启后，在用户重新加载或首次发送前通过 `session/load` 恢复同一 Agent session，并用 thread 回放对账本地镜像。交付状态不确定时必须先对账，避免重复发送。
- 支持流式文本、安全 Markdown（标题、强调、列表、引用、表格、代码）、完整的 KaTeX 0.18.4 → Firefox MathML 公式，以及 fenced `mermaid` 图表。Mermaid 11.16.1 随 XPI 离线内置并按窗口延迟加载；图表以严格模式、禁用 HTML 标签和交互的方式渲染，经本地资源与 SVG 白名单复核后作为隔离数据图片显示，宽图可横向滚动，源码可折叠查看和复制，解析失败或超限时自动展开源码。公式支持分式、求和上下标、集合运算、重音、根式、矩阵、对齐环境及上下花括号；解析失败时保留原始 TeX，不再输出命令粘连的伪公式。Codex 的 `:codex-file-citation{...}` 与兼容的 `::codex-file-citation{...}` 文件引用会显示为引用胶囊，并且只允许在当前论文工作区内定位。Codex/Pi 单工具原始输出统一限制为 64 KiB；流式文本与思考按 50 ms 合并刷新，镜像写盘按 500 ms 防抖，状态快照只深拷贝一次。折叠工具卡片不再预建大输出 DOM，展开后才生成内容；长对话中仍保留已展开卡片与阅读位置，只有用户原本就在底部时才继续跟随新内容。
...
- `execute`、文件读取、图片查看和搜索等常见工具会显示为语义卡片，只呈现安全元数据，不再把内部 ID、时间戳及原始事件 JSON 暴露在界面中；权限审批使用同一套可读展示。Codex 完成态 `View Image` 会联合核对工具类型、标题、输入路径、位置和资源链接；Windows 原生盘符路径、正斜杠形式与 `file:///C:/...` 会在比较时统一，但文件读取仍使用原生绝对路径。通过常规文件、25 MiB、扩展名与文件签名校验的 PNG/JPEG/GIF/WebP/AVIF 会复制到工作区外的会话媒体目录；SVG、未知格式、路径不一致和伪装文件只显示错误，不回退直读源路径。图片卡片默认折叠，用户展开后才解码本地副本；点击预览可在当前 Zotero 窗口放大，并在适应窗口与 1:1 原始像素间切换。Web Search 会区分多查询搜索、打开网页和页内查找，完整展示 ACP 返回的查询、页面、查找词，以及事件中实际携带的结果标题、摘要或文本；若固定适配器没有传回网页正文或结果摘要，卡片会明确标注协议事件未携带内容。
...
0.1.36 已在 Windows 上通过 259 项自动测试、全部 JavaScript 语法检查、静态安全检查、可复现构建、SHA-256、XPI 根目录与 `unzip -t` 检查；归档内只有 38 个运行时文件，且与 `plugin/` 源文件逐字节一致。回归覆盖 Windows 盘符/UNC 绝对路径、拒绝盘符相对路径、分号 PATH、`node.exe`、标准 npm `npx-cli.js`、nvm-windows 目录发现，以及 Zotero 当前 PDF 源文件在 Windows/Unix 上的存在、缺失和非 PDF 分支。Windows Zotero 9.0.6 只读诊断确认当前 Reader 精确映射的 PDF 附件源文件真实存在；此前错误由仅接受 `/` 的源路径判断造成，当前实现改为复用跨平台绝对路径校验后再确认文件存在。`pdftotext` 探测现按平台从 PATH 构造可执行文件候选，Windows 不再探测 `/opt/homebrew`；任一候选的路径解析或文件访问异常都会被隔离，并在不可用时继续回退 Zotero PDFWorker。Windows View Image 将原生盘符路径与正斜杠/file URI 等价形式分开处理：统一比较键用于多来源印证，原始可用路径用于文件读取和复制，避免绝对路径被再次拼接到工作区；真实失败会话中的 PNG 路径与签名已通过只读重放验证。性能回归覆盖 Codex/Pi 统一的 64 KiB 单工具输出上限、旧镜像一次性限幅、公开快照隔离、50 ms 流式合并不丢字，以及折叠工具输出延迟构建；当前真实会话的只读迁移演算由约 582 KiB 降至 435 KiB，减少 25.2%。最终同版本重建 XPI 已通过 `AddonManager.getInstallForFile()` 非安装式解析：插件 ID 为 `smart-paper-translator@zotero.local`，版本为 `0.1.36`，`error: 0`、`isCompatible: true`、`appDisabled: false`；未安装该重建包或调用模型。此前安装并由用户在设置页准备后，只读核对确认 Node 24.13.1、npx 11.8.0、Codex CLI 0.153.0 与固定 `codex-acp 1.6.2` 均使用所选 Windows 绝对路径；准备版本及路径指纹相互匹配，固定适配器可在 npm 离线模式下启动，ACP 握手、ChatGPT 登录、健康状态、能力与模型选项目录均正常。
